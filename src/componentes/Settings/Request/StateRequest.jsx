...
export default function StateRequest() {
  ...
  const filtroRequestsUsers = allRequests
    .filter((e) => e.services?.user?.email === user?.email)
    .reverse();
  ...
  const handleOnClick = (e) => {
    if (btn.state === "") {
      ...
      let prev = document.getElementById(btn.state);
      prev.style.cssText = 'background-color: "#1F2937";';
      if (e.target.name === "rechazado") {
        e.target.style.cssText = "background-color: red;";
      } else {
        e.target.style.cssText = "background-color: green;";
      ...
      e.target.style.cssText = 'background-color:"#1F2937";';
      btn.state = "";
    }
  };
