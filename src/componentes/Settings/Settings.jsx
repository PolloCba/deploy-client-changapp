...
export default function Settings(id) {
  ...
  const totalNotifications = Notifications.filter(
    (e) => e.userNotificated_id === userData?.id
  ).length;
  const totalServices = userData?.services ? userData?.services.length : 0;
  let totalRequestsReceived = 0;
  userData?.services?.forEach(
    (e) => (totalRequestsReceived += e.request.length)
  );
          ...
          <NavLink style={(e) => handleSelected(e)} to="reviews">
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <RateReviewIcon
                id="servicesIcon"
                style={
                  location?.pathname === "/settings/reviews"
                    ? styles.icons2
                    : styles.icons
                }
              />
          ...
          <NavLink style={(e) => handleSelected(e)} to="request">
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <EmailIcon
                id="requestIcon"
                style={
                  location?.pathname === "/settings/request"
                    ? styles.icons2
                    : styles.icons
                }
              />
          ...
          <NavLink style={(e) => handleSelected(e)} to="requester">
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <SendIcon
                id="requesterIcon"
                style={
                  location?.pathname === "/settings/requester"
                    ? styles.icons2
                    : styles.icons
                }
              />
